...
export const HEADER = `  ${CYAN}~ collapse-siren ~${RESET}  ${DIM}siren blares when collapse rate spikes${RESET}`;
...
export function renderSiren(rows: AgentRow[], threshold: number, justTripped: string | null): string {
  ...
  for (const r of rows) {
    ...
    const mark = tripped ? (flashing ? `${YELLOW}${BOLD}▲ ALERT!${RESET}` : `${RED}▲${RESET}`) : " ";
