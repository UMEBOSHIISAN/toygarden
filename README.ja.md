...
9つの部品（`packages/core-*`）を、1枚の契約（`contracts/`）でつなぐと、
...
> **数え方の注記:** `core-*` パッケージは物理的には10個ある。うち9個が遊びを組み立てる部品で、
> 10個目の `core-termgif` はデモ GIF を焼くメタ部品（[デモが腐らない仕組み](#デモが腐らない仕組み)）。
> 「9つの部品で22の遊び」は前者の数え方。
...
npm run hello            # 冒頭に映画的スプラッシュ、そのあと Undertale 風の案内役が中へ（TOYGARDEN_NO_SPLASH=1 で省略）
npm run tour             # 座って眺めるだけ。全22本を8秒ずつ、セーブポイント風の幕間つき
...
npm run check            # typecheck + テスト（150本超・全緑。おもちゃが増えるほど増える）
...
> **`npx toygarden` は準備中。** bin エントリと事前バンドルは仕込み済みだが、パッケージは
...
`core-*` は10個。うち9個が遊びを組み立て、10個目の `core-termgif` は全デモ GIF を焼くメタ部品。
...
| [`core-save`](packages/core-save/) | `~/.toygarden` への fail-soft な JSON 永続化（小さな状態用・依存ゼロ） |
| [`core-sysmon`](packages/core-sysmon/) | ホスト負荷（CPU/メモリ/load を `node:os` で）を 0..1 の busyness に正規化 |
| [`core-termgif`](packages/core-termgif/) | ANSI出力→GIF。デモを腐らせないための部品（GIF89a+LZW+8x8フォント内蔵） |

## 遊びカタログ（apps/ — 全22本・全部GIF付き）
...
| [collapse-arcade](demo/gifs/collapse-arcade.gif) | worker-data | 崩壊率の高いエージェントが敵に。撃墜=レビュー。ベストスコアは RPG セーブ風に `~/.toygarden/save.json` へ記録 — 🏆 Issue テンプレで自慢できる |
...
| [cpu-diner](demo/gifs/cpu-diner.gif) | sysmon | マシンの負荷が営む ASCII 食堂。CPU が忙しいと客が押し寄せ、暇だと店員が居眠りする |

どの app も `apps/<name>/src/` の数ファイル。既存を一切壊さず追加できる = このキットの拡張性そのもの。

## ガジェットを繋ぐ

全22本が **実機ゼロ**で動く — `core-device` の既定が mock ドライバで、その既定こそが
全 app・全 CI を「何も挿さずに」回せる理由。でも `Device` HAL は本物だ。小さな画面と
ボタンが1つあれば、toygarden は実機パネルを駆動できる。
...
toygarden は human と AI エージェントが並んで作った。git 履歴には今も `Co-Authored-By`
...
スクリーン録画の GIF はコードが変わった瞬間に嘘になる。toygarden の GIF は全部こう作る:
...
書き換えて育てる。あとは `package.json` に `@toygarden/*` core を足して部品を掛け合わせるだけ。

手動で足す場合や、新しい core / デバイス / イベントの足し方は **[CONTRIBUTING.md](CONTRIBUTING.md)** を参照。

## 組み合わせのフロンティア

10個の `core-*` 部品は **45通り**にペアを組める — そしてほとんどのマスはまだ空いている。
この地図は `npm run frontier` が `manifest.json` から実計算して描くので、嘘をつけない —
2つの部品を本当に掛け合わせたおもちゃがある時だけマスが光り、**新しい部品が増えるたびに
地図が描き直される**。`?` が明滅する暗いマスが、まだ誰も作っていない組み合わせだ。

![組み合わせのフロンティア](demo/frontier.gif)

> **45通りの組み合わせ、その大半はまだ空。空きマスはあなたのもの。**
...
core-termgif の LZW は独立実装デコーダとの roundtrip でテストしている。**150本超・全緑
