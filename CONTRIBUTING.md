...
足場作りには2つの入口がある。どちらも生成物は同じ「動く最小のおもちゃ」一式:

- **視覚派 → `npm run workshop`**（対話 TTY）。`packages/` の `core-*` を部品カードとして
  並べ、`j`/`k`+`space` で選ぶと画面下のレシピ図がライブで組み変わる。名前を決めて `Enter`
  すると、選んだ部品の配線サンプル入りで `apps/<name>/` が生える（scaffold の実体は下の
  `new-toy` を再利用。既存おもちゃと同じ部品構成なら「おなじ血筋!」と教えてくれる）。
- **CLI 派 → `npm run new`**。非対話環境（CI・パイプ経由）や、部品を後で足す前提で
  最小の足場だけ欲しいときはこちら。

以下は CLI 派の手順。いちばん速い足場作りは scaffolder に任せる。**約60秒**で一式が出る:
...
## 新しい core（部品）を足す — 写経手順

**ここが拡張性の本丸。** 部品を1個足すと、既存の全おもちゃとの掛け合わせが一気に増える
（1 core → N 通りの新しい遊び）。だから新しい遊びを作るのと同じ粒度で、手順を固定しておく。

いちばん小さい core は `packages/core-events/`（5ファイル）。これを見本に写経すれば、どの core も
同じ形になる。まずその中身を丸ごと写す:
...
│   └── bus.ts       # 純ロジック本体（テスト対象）
├── test/
│   └── bus.test.ts  # 純ロジックの vitest unit
└── README.md        # 責務1文 / 提供API表 / 使用例 / 使っている app / 設計原則
```

`package.json` の中身はこれだけ（`<domain>` を自分の部品名に置き換える。依存は `contracts` 1本のみ）:
...
1. `packages/core-<domain>/` を作り、上のツリーを写す。`package.json` の `name` と依存だけ書き換える
...
このリポジトリ自体が human × AI の共作で育っている（git 履歴の `Co-Authored-By` 参照）。
あなたが AI コーディングエージェントと組んで部品やおもちゃを作る場合のコツ:

- **package 境界 = タスク境界。** 1 core（または 1 app）= 1 タスクで切ると、エージェントは迷わない
- `contracts/events.ts` と対象 core の spec + test を**先に**固めてから実装を任せる
- 仕上げに必ず自分で `npm run check` と glyph lint を実測する（エージェントの「通りました」を信用しない）
