...
```ts
import { commitsSince, toPlayEvents } from "@umeplay/core-git-observe";
...
入力に使う「掛け合わせ」が umeplay の基本パターン。

## 使っている app

`commit-constellation` / `commit-symphony` / `git-replay` / `git-weather` が
`@umeplay/core-git-observe` を直接 import する（`apps/*/src/*.ts` を grep して実測）。
