# @toygarden/core-worker-data

worker 配車ログ（routing trial）と崩壊統計（collapse stats）を **read-only で供給**する部品。
TSV ファイルをパースして構造化データと `PlayEvent` の両方を返す。依存は `@toygarden/contracts` のみ。

研究・運用データ（worker_routing_ledger 等）を toygarden の遊びに繋ぐための「読むだけ」アダプタ。
mutation は一切行わない。
...
```ts
import { loadCollapseStats, collapseToEvents } from "@toygarden/core-worker-data";
...
`@toygarden/core-worker-data` を直接 import する（`apps/*/src/*.ts` を grep して実測）。
